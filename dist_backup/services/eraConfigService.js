...
class EraConfigService {
    ...
    async getChips() {
        try {
            ...
            console.log("EraConfigService: Chips response:", response.status, responseData);
    ...
    async getDatastreams(chipId) {
        try {
            console.log("EraConfigService: Fetching datastreams", chipId ? `for chip ${chipId}` : "for all chips");
            if (!this.authService || !this.authService.isAuthenticated()) {
                return {
                    success: false,
                    error: "Not authenticated",
            ...
            const responseData = await response.json();
            console.log("EraConfigService: Datastreams response:", response.status, responseData);
            if (response.ok && responseData) {
                ...
                console.error("EraConfigService: Failed to fetch datastreams:", responseData);
                return {
                    success: false,
                    error: responseData.message ||
                        responseData.error ||
    ...
    async getCompleteConfig() {
        try {
            ...
            return {
                ...
                message: `Found ${chipsResult.chips?.length || 0} chips and ${datastreamsResult.datastreams?.length || 0} datastreams`,
            };
        }
        catch (error) {
    ...
    parseChipsResponse(responseData) {
        ...
        try {
            ...
            console.log(`EraConfigService: Parsed ${chips.length} chips from response`);
        }
        catch (error) {
        ...
        try {
            ...
            if (Array.isArray(streamData)) {
                streamData.forEach((stream) => {
                    ...
                    const streamChipId = stream.chip_id || stream.chipId || stream.device_id;
            ...
            console.log(`EraConfigService: Parsed ${datastreams.length} datastreams from response`);
        }
        catch (error) {
            console.error("EraConfigService: Error parsing datastreams response:", error);
        }
        return datastreams;
    }
    /**
    ...
    updateMapping(sensorType, datastreamId) {
        ...
        console.log(`EraConfigService: Updated mapping for ${sensorType}:`, datastreamId);
    }
    /**
...
exports.default = EraConfigService;
