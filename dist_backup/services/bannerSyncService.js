...
class BannerSyncService {
    ...
    subscribeToTopics() {
        ...
        topics.forEach((topic) => {
            this.mqttClient.subscribe(topic, { qos: 1 }, (err) => {
                if (err) {
                    console.error(`BannerSyncService: Failed to subscribe to ${topic}:`, err);
                }
                else {
    ...
    async handleBannerUpdate(data) {
        try {
            ...
            if (data.type === "banner_update" && data.data) {
                ...
                const localPath = await this.downloadBanner(bannerData.url, bannerData.id);
                if (localPath) {
                    // Update local cache
                    ...
                    console.log("BannerSyncService: Banner updated successfully:", bannerData.id);
                }
            }
        }
        catch (error) {
    ...
    async handleBannerDelete(data) {
        try {
            ...
            if (data.type === "banner_delete" && data.bannerId) {
                ...
                console.log("BannerSyncService: Banner deleted successfully:", bannerId);
            }
        }
        catch (error) {
    ...
    async handleBannerSync(data) {
        try {
            ...
            if (data.type === "settings_sync" && data.settings) {
                ...
                console.log("BannerSyncService: Settings synced successfully:", settings);
            }
        }
        catch (error) {
     ...
     */
    async downloadBanner(url, bannerId) {
        try {
            ...
            return new Promise((resolve, reject) => {
                writer.on("finish", () => {
                    console.log("BannerSyncService: Banner downloaded successfully:", localPath);
                    resolve(localPath);
                });
     ...
     */
    async updateLocalBannerConfig(bannerData, localPath) {
        try {
            ...
            if (typeof window !== "undefined" && window.electronAPI) {
                ...
                const filteredBanners = (currentConfig.logoImages || []).filter((img) => img.id !== bannerData.id);
                // Add new banner
     ...
     */
    async removeBannerFromConfig(bannerId) {
        try {
            if (typeof window !== "undefined" && window.electronAPI) {
                const currentConfig = await window.electronAPI.getConfig();
                // Remove banner with matching ID
                const filteredBanners = (currentConfig.logoImages || []).filter((img) => img.id !== bannerId);
                // Update config
                const updatedConfig = {
                    ...currentConfig,
                ...
                // Save updated config
                await window.electronAPI.saveConfig(updatedConfig);
                console.log("BannerSyncService: Banner removed from local config:", bannerId);
            }
        }
        catch (error) {
            console.error("BannerSyncService: Error removing banner from config:", error);
        }
    }
    /**
     * Update local settings
     */
    async updateLocalSettings(settings) {
        try {
            if (typeof window !== "undefined" && window.electronAPI) {
                const currentConfig = await window.electronAPI.getConfig();
                ...
                const updatedConfig = {
                    ...
                    logoLoopDuration: settings.loopDuration || currentConfig.logoLoopDuration,
                };
                // Save updated config
                await window.electronAPI.saveConfig(updatedConfig);
...
exports.default = BannerSyncService;
