...
function enhanceLogoManifestDebugging() {
    ...
    if (typeof window !== "undefined") {
        window.addEventListener("logo-manifest-updated", (event) => {
            ...
            if (manifest) {
                console.log(`📊 Manifest info: ${manifest.logos?.length || 0} logos, version: ${manifest.version}`);
                // Log active logos
                const activeLogos = manifest.logos?.filter((logo) => logo.active) || [];
                console.log(`✅ Active logos: ${activeLogos.length}`, activeLogos.map((logo) => logo.name));
                // Check for broken URLs
                const brokenLogos = manifest.logos?.filter((logo) => !logo.url ||
                    logo.url.includes("company-logo-3.png") ||
                    logo.url.includes("/blob/")) || [];
                if (brokenLogos.length > 0) {
                    console.warn("⚠️ Potentially broken logos detected:", brokenLogos.map((logo) => ({
                        id: logo.id,
                        name: logo.name,
                        url: logo.url,
                    })));
...
async function testLogoDownloads() {
    ...
    try {
        const manifestUrl = "https://mquan-eoh.github.io/billboard-logos-cdn/manifest.json";
        ...
        for (const logo of manifest.logos || []) {
            ...
            try {
                ...
                if (logoResponse.ok) {
                    ...
                    console.error(`❌ Logo ${logo.id} failed: ${logoResponse.status} ${logoResponse.statusText}`);
                }
            }
            catch (error) {
...
function getSystemState() {
    const state = {
        ...
        logoManifestService: typeof window !== "undefined" && !!window.logoManifestService,
...
console.log("💡 Available functions: enhanceLogoManifestDebugging(), testLogoDownloads(), forceTriggerLogoUpdate(), getSystemState()");
