...
class BillboardConfigManager {
    ...
    async initAuthService() {
        try {
            ...
            if (!window.EraAuthService) {
                console.error("ConfigManager: EraAuthService not found on window object");
                return;
            }
    ...
    async initEraConfigService() {
        try {
            ...
            if (!window.EraConfigService) {
                console.error("ConfigManager: EraConfigService not found on window object");
                return;
            }
            this.eraConfigService = new window.EraConfigService(this.authService);
            console.log("ConfigManager: E-Ra config service initialized successfully");
        }
        catch (error) {
            console.error("ConfigManager: Failed to initialize E-Ra config service:", error);
        }
    }
    init() {
    ...
    handleLogout() {
        ...
        this.showNotification("Logged out successfully! You can now login with a different account.", "success");
        ...
        setTimeout(() => {
            ...
            if (loginForm) {
                ...
                const formControls = loginForm.querySelectorAll(".form-control, button");
    ...
    updateLoginUI(authState) {
        ...
        if (authState.isAuthenticated) {
            ...
            statusText.textContent = `Đăng nhập với ${authState.user?.username || "User"}`;
    ...
    async handleGetEraConfig() {
        ...
        if (statusText)
            statusText.textContent = "Đang lấy cấu hình";
        try {
            ...
            if (result.success) {
                ...
                if (statusIndicator)
                    statusIndicator.className = "status-indicator online";
                if (statusText)
                    statusText.textContent = `Configuration loaded: ${result.chips?.length || 0} chips, ${result.datastreams?.length || 0} datastreams`;
            }
            else {
                this.showNotification(result.message || "Failed to fetch configuration", "error");
                // Update status
                if (statusIndicator)
                    statusIndicator.className = "status-indicator offline";
                if (statusText)
            ...
            this.showNotification("Error fetching configuration: " + error.message, "error");
            // Update status
            if (statusIndicator)
                statusIndicator.className = "status-indicator offline";
            if (statusText)
                statusText.textContent = "Configuration fetch failed";
        }
        finally {
            // Reset button state
    ...
    handleAutoDetectMapping() {
        ...
        const mappedCount = Object.values(autoMapping).filter((id) => id !== null).length;
        this.showNotification(`Auto-detected ${mappedCount}/4 sensor mappings`, "success");
    }
    async handleSaveMapping() {
        ...
        // Validate mapping
        const mappedCount = Object.values(currentMapping).filter((id) => id !== null).length;
        if (mappedCount === 0) {
            this.showNotification("Please map at least one sensor before saving", "error");
            return;
        }
        ...
        this.showNotification(`Sensor mapping saved and applied instantly! (${mappedCount}/4 sensors mapped)`, "success");
    }
    async handleTestMapping() {
        ...
        const currentMapping = this.eraConfigService.getCurrentMapping();
        const mappedSensors = Object.entries(currentMapping).filter(([, id]) => id !== null);
        if (mappedSensors.length === 0) {
            this.showNotification("No sensor mappings to test", "error");
            return;
        }
        this.showNotification(`Testing ${mappedSensors.length} sensor mappings...`, "info");
        ...
        if (successCount === mappedSensors.length) {
            this.showNotification("All sensor mappings tested successfully!", "success");
        }
        else {
            this.showNotification(`${successCount}/${mappedSensors.length} sensor mappings working`, "error");
        }
    }
    displayChips(chips) {
      ...
      `;
        ...
        <div>
          ...
          <p style="margin: 0; font-size: 14px; color: #6c757d;">ID: ${chip.id}${chip.description ? ` | ${chip.description}` : ""}</p>
        </div>
        <div style="text-align: right;">
          <span class="status-indicator ${chip.isOnline ? "online" : "offline"}" style="font-size: 14px;">
    ...
    displayDatastreams(datastreams) {
        ...
        if (!datastreamsList || !datastreams)
            return;
      ...
      `;
        ...
        <div>
          ...
          <span style="margin-left: 10px; color: #6c757d;">ID: ${stream.id}</span>
          ${stream.unit
    ...
    populateMappingSelectors(datastreams) {
        ...
        sensorTypes.forEach((sensorType) => {
            ...
            datastreams.forEach((stream) => {
                ...
                option.textContent = `${stream.name} (ID: ${stream.id})${stream.unit ? ` [${stream.unit}]` : ""}`;
    ...
    updateCurrentMappingDisplay() {
        ...
        if (!displayDiv || !this.eraConfigService)
            return;
        const currentMapping = this.eraConfigService.getCurrentMapping();
      ...
      `;
            const datastreamName = datastreamId
                ...
                : "Not mapped";
            const sensorTypeLabel = sensorType.charAt(0).toUpperCase() + sensorType.slice(1);
    ...
    updateUI() {
        ...
        const modeRadio = document.querySelector(`input[value="${this.config.logoMode}"]`);
        if (modeRadio) {
    ...
    renderLogoGrid() {
        ...
        this.config.logoImages.forEach((logo, index) => {
            ...
            console.log(`Config: Processing logo ${logo.name}, original path: ${logo.path}, source: ${logo.source}`);
    ...
    addScheduleRule() {
        ...
        ruleDiv.innerHTML = `
            ...
            .map((logo, idx) => `<option value="${idx}">${logo.name}</option>`)
            .join("")}
                </select>
            </div>
            <div style="flex: 1;">
...
async function saveAndApply() {
    ...
    configManager.showNotification(`Configuration saved! Logo rotation: ${configManager.config.logoLoopDuration}s interval.`, "success");
