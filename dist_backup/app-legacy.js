...
class LogoManifestService {
    ...
    async initialize(config) {
        ...
        try {
            ...
            console.log(`LogoManifestService: Poll interval: ${config.pollInterval}s`);
    ...
    async ensureDownloadDirectory() {
        try {
            if (window.electronAPI && window.electronAPI.ensureDirectory) {
                await window.electronAPI.ensureDirectory(this.config.downloadPath + "/logos");
                console.log("LogoManifestService: Download directory ensured");
            }
        }
        catch (error) {
            console.warn("LogoManifestService: Could not ensure download directory:", error);
        }
    }
    startPolling() {
    ...
    async fetchAndProcessManifest() {
        ...
        try {
            ...
            if (manifest) {
                ...
                console.log(`LogoManifestService: Active logos: ${manifest.logos.filter((l) => l.active).length}`);
                // Check if manifest has changed
                if (this.hasManifestChanged(manifest)) {
                    console.log("LogoManifestService: 🔄 Manifest has changed, processing updates...");
            ...
            if (this.retryCount < (this.config.retryAttempts || 3)) {
                console.log(`LogoManifestService: Retrying in ${this.config.retryDelay || 2000}ms... (${this.retryCount}/${this.config.retryAttempts})`);
                setTimeout(() => {
                    this.fetchAndProcessManifest();
    ...
    hasManifestChanged(newManifest) {
        ...
        return (this.manifestCache.version !== newManifest.version ||
    ...
    async processLogoDownloads(manifest) {
        ...
        for (const logo of activeLogos) {
            try {
                ...
                console.error(`LogoManifestService: Failed to download logo ${logo.name}:`, error);
            }
        }
    }
    async downloadLogoIfNeeded(logo) {
        ...
        try {
            ...
            console.error(`LogoManifestService: Download check failed for ${logo.name}:`, error);
        }
    }
    async downloadLogo(logo, localPath) {
        try {
            ...
            if (window.electronAPI && window.electronAPI.writeFile) {
                ...
                console.log(`LogoManifestService: ✅ Downloaded ${logo.name} to ${localPath}`);
            }
            else {
            ...
            console.error(`LogoManifestService: Download failed for ${logo.name}:`, error);
            throw error;
        }
    }
        ...
        try {
            if (window.electronAPI &&
                ...
                const logoImages = manifest.logos
                    ...
                    console.log(`Logo path mapping: ${logo.filename} -> ${absolutePath}`);
                    return {
                        name: logo.filename,
            ...
            console.error("LogoManifestService: Failed to update local config:", error);
        }
    }
    ...
    try {
        if (window.electronAPI && window.electronAPI.getConfig) {
            ...
            if (config.logoManifest && config.logoManifest.enabled) {
                ...
                const initialized = await globalLogoManifestService.initialize(config.logoManifest);
                if (initialized) {
...
window.addEventListener("load", () => {
    if (!globalLogoManifestService || !globalLogoManifestService.isInitialized) {
        console.log("Window loaded, initializing Logo Manifest Service as backup...");
        initializeLogoManifestService();
    }
});
// ====================================
...
class WeatherPanel extends React.Component {
    ...
    render() {
        ...
        }, weatherData.city), React.createElement("div", { style: { fontSize: "32px", fontWeight: "bold" } }, `${weatherData.temperature}°`), React.createElement("div", { style: { fontSize: "12px" } }, `${weatherData.lowTemp}°`), React.createElement("div", { style: { fontSize: "12px" } }, `Độ ẩm ${weatherData.humidity}% | UV ${weatherData.uvIndex}`), React.createElement("div", { style: { fontSize: "12px" } }, `Mưa ${weatherData.rainChance}% | Gió ${weatherData.windSpeed} km/h`), React.createElement("div", {
            style: {
                fontSize: "10px",
                marginTop: "5px",
            },
...
class IoTPanel extends React.Component {
    ...
    render() {
        ...
        }, "THIẾT BỊ ĐO"), React.createElement("div", { style: { fontSize: "14px", marginBottom: "4px" } }, `Nhiệt độ: ${sensorData.temperature}°`), React.createElement("div", { style: { fontSize: "14px", marginBottom: "4px" } }, `Độ ẩm: ${sensorData.humidity}%`), React.createElement("div", { style: { fontSize: "14px", marginBottom: "4px" } }, `PM2.5: ${sensorData.pm25} μg`), React.createElement("div", { style: { fontSize: "14px", marginBottom: "4px" } }, `PM10: ${sensorData.pm10} μg`), React.createElement("div", {
            style: {
                fontSize: "10px",
...
class CompanyLogo extends React.Component {
    ...
    componentWillUnmount() {
        ...
        if (this.logoManifestHandler) {
            window.removeEventListener("logo-manifest-updated", this.logoManifestHandler);
        }
    }
    async loadConfig() {
        try {
        ...
        if (config &&
            ...
            this.logoRotationInterval = setInterval(() => {
                this.setState((prevState) => ({
                    currentLogoIndex: (prevState.currentLogoIndex + 1) % config.logoImages.length,
                }));
            }, duration);
        }
    }
    ...
    renderDefaultLogo() {
        return [
            ...
            }, "CÔNG TY"), React.createElement("div", {
                style: {
                    fontSize: "12px",
                    lineHeight: "1.2",
                    margin: 0,
    ...
    render() {
        ...
        return React.createElement("div", { style: containerStyle }, currentLogo
...
class BillboardLayout extends React.Component {
    render() {
        ...
        return React.createElement("div", { style: containerStyle }, React.createElement("div", { style: topRowStyle }, React.createElement(WeatherPanel), React.createElement(IoTPanel)), React.createElement(CompanyLogo));
    }
}
class App extends React.Component {
    render() {
        ...
        return React.createElement("div", { style: appStyle }, React.createElement(BillboardLayout));
    }
}
