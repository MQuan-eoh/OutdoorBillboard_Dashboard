...
class BillboardApp {
    ...
    updateLogoDisplay(container = null) {
        const logoSection = container || document.getElementById("company-logo-section");
        if (!logoSection)
            return;
    ...
    updateWeatherData() {
        ...
        if (weatherPanel) {
            ...
            weatherPanel.innerHTML = `
                ...
                <div style="font-size: 12px;">Mưa ${Math.floor(Math.random() * 100)}% | Gió ${(Math.random() * 5).toFixed(1)} km/h</div>
                <div style="font-size: 10px; margin-top: 5px;">Chất lượng không khí: Tốt</div>
            `;
        }
    }
